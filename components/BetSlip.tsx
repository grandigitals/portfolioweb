...
import { X, Trash2, ChevronRight, AlertCircle, CheckCircle, Wallet, ShoppingCart, LogIn } from 'lucide-react'
...
export default function BetSlip({ mode = 'both' }: { mode?: 'sidebar' | 'mobile' | 'both' }) {
  ...
  const sharedProps = {
    betSlip, placedBets, balance,
    activeTab, setActiveTab,
    message, totalStake, potentialReturn,
    removeFromBetSlip, updateStake,
    clearBetSlip, handlePlaceBet,
    user,
  ...
  if (mode === 'mobile') {
    return (
      <>
        ...
        >
          <BetSlipContent {...sharedProps} showCloseButton />
        </aside>
...
function BetSlipContent({
  ...
  return (
    <>
      ...
      {/* Sign-in notice banner when logged out */}
      {!user && (
        <div className="px-4 py-3 bg-accent-green/10 border-b border-accent-green/30 flex items-center gap-2">
          <LogIn className="w-4 h-4 text-accent-green flex-shrink-0" />
          <p className="text-xs text-accent-green font-medium">Sign in to view your balance &amp; place bets</p>
        </div>
      )}
          ...
          <div className="p-3 space-y-3">
            {!user ? (
              <div className="flex flex-col items-center justify-center h-48 text-gray-500 px-4">
                ...
                <p className="text-sm text-center">Sign in to view your bet history</p>
              </div>
            ) : placedBets.length === 0 ? (
              <div className="flex flex-col items-center justify-center h-48 text-gray-500 px-4">
      ...
      {activeTab === 'slip' && betSlip.length > 0 && (
        <div className="p-4 border-t border-dark-border bg-dark-card space-y-3">
          ...
          {/* Summary — only show totals when logged in */}
          {user && (
          ...
          <div className="flex gap-2">
            ...
            {user ? (
              /* Logged in — normal Place Bet button */
              <button
              ...
              /* Not logged in — sign-in prompt button */
              <button
                onClick={handlePlaceBet}
                className="flex-1 flex items-center justify-center gap-2 py-2.5 bg-accent-green hover:bg-accent-green/90 text-dark-bg font-bold rounded-lg transition-all duration-200 text-sm"
              >
